...
const CartItem = ({ item: { id,   name, image, color, shadow, price, cartQuantity } }) => {
  const dispatch = useDispatch();

  const onRemoveItem = () => {
    dispatch(setRemoveItemFromCart({ id,   name, image, color, shadow, price, cartQuantity }))
  }

  const onIncreaseItemQTY = () => {
    dispatch(setIncreaseItemQTY({ id,   name,  image, color, shadow, price, cartQuantity }))
  }
  const onDecreaseItemQTY = () => {
    dispatch(setDecreaseItemQTY({ id,   name,  image, color, shadow, price, cartQuantity }))
  }
        ...
        <div className="flex items-center gap-5">
          ...
          <div className="grid items-center gap-4">
            <div className="grid items-center leading-none">
              ...
              <p className="text-sm text-slate-800 lg:text-xs">SNEAKINN </p>
            </div>
