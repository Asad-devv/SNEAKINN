...
const getPopularProduct = (id) => {
  popularsales.items.find((product) => product.id === id);
};
const ProductPage = () => {
  ...
  useEffect(() => {
    if (id.includes("-")) {
      ...
      if (id.includes("rated")) {
        getPopularProduct(getProductById(id));
  ...
  const onAddToCart = () => {
    const item = {
      ...
      image: product.image,
      price: `${
        product?.traits.find((trait) => trait.name === "retail-price")?.value
      }`,
    };
