...
const Item = ({
 ...
 const onAddToCart = () => {
  ...
  return (
    <>
      <div
        className={`relative bg-gradient-to-b bg-gradient-to-b from-fuchsia-50 to-indigo-400 white shadow-lg shadow-violet-500 ${shadow} grid h-[40vh] sm:h-[50vh] items-center justify-items-center rounded-md py-4 px-5 transition-all duration-700 ease-in-out w-full hover:scale-105 my-3 mb-4`}
        // style={{ height: "300px" }}
      >
        <div
        ...
        >
          <LazyLoad 
            ...
            // alt={`img/item-img/${id}`}
            className={`transitions-theme sm:w-[8rem] sm:h-[auto] h-36 w-64`}
            // placeholder={<img  className={`transitions-theme h-36 w-64`} src={image} alt="image" />}
          >
        <img loading="lazy" className={`transitions-theme  sm:w-[7rem] h-36 w-64`} src={image} alt="image" />  
