...
const ProductPage = () => {
  ...
  const findPrice =  (traits) => {
    const retailPrice =  traits.find(
    ...
    if (id.includes("-")) {
      ...
      console.log("loru");
        getProductsBySlug(id).then((data) => {
          console.log(data);
          setProduct(data);
          findPrice(product?.traits).then((price) => {
    ...
    <Grid container height="100vh">
      ...
      >
        ...
        <Typography variant="subtitle1">
          {!isDemo ?  ( retailPrice) && (<p>Retail Price: {retailPrice.value}</p>)  : product?.price}$
        </Typography>
        <Box display="flex">
          <Box
            display="flex"
