...
const SearchBar = ({
  ...
  return (
    <Box display="flex" flexDirection="column">
      <Box sx={{ width: {sm:"15vh",md:"20vh",lg:"40vh"}, borderRadius: "30px" }}>
        <TextField
          ...
          InputProps={{
            style: {
              borderRadius: "60px",
              // height:"52px"
              width: {sm:"15vh",md:"20vh",lg:"40vh"}
            },
            endAdornment: (
              <IconButton onClick={handleIconClick}>
                <Link to={`/products/search/${searchValue}`}>
                  
                  <SearchIcon p={5} sx={{border :"1px solid black",backgroundColor:"#ccc7c7",borderRadius:"50%",fontSize:"30px"}} />
                </Link>
              </IconButton>
            ),
          }}
      ...
      {filteredProducts.length > 1 &&
        ...
        suggestionsOpen && (
          ...
          >
            ...
            <Typography><Link to={`/products/search/${searchValue}`}>ASAD</Link></Typography>
          </Box>
        )}
    </Box>
  );
