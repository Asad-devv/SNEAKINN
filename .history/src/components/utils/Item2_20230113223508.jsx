...
const Item = ({
 ...
 const onAddToCart = () => {
  ...
  return (
    <>
      <div
        className={`relative  bg-gradient-to-t from-fuchsia-50 to-blue-100 white shadow-lg shadow-gray-300  grid h-[42vh] sm:h-[50vh] items-center justify-items-center rounded-md py-4 px-5 transition-all duration-700 ease-in-out w-full hover:scale-105 my-3 mb-4`}
        // style={{ height: "300px" }}
      >
        <div
        ...
        >
          ...
          numbers.map((number, index) => (
            <>
            <h1 key={index} className="text-black text-sm font-medium">{number}</h1>
            {console.log(number)}</>
        ...
        <div
          className={`flex items-center sm:w-[10rem]  pb-2 justify-center p-0.5 rounded-lg`}
        ...
        >
          <LazyLoad 
            ...
            // alt={`img/item-img/${id}`}
            className={`transitions-theme  pb-2 sm:w-[8rem] sm:h-[auto] h-37 w-64`}
            // placeholder={<img  className={`transitions-theme h-36 w-64`} src={image} alt="image" />}
          >
        <img loading="lazy" className={`transitions-theme  sm:w-[9rem]  pb-2 h-37 w-64`} src={image} alt="image" />  
