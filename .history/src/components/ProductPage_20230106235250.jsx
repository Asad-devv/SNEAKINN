...
const ProductPage = () => {
  ...
  const findPrice =async (traits)=>{
    const retailPrice = await traits.find((trait) => trait.name === "retail-price");
    console.log(retailPrice)
    // const retailPri-ce = findPrice(product.traits);
if (retailPrice) {
  console.log(`Retail price: ${retailPrice.value.value}`);
}
  }
useEffect(() => {
  if(id.includes("-")){
  ...
  return (
    ...
    <Grid container height="100vh">
      ...
      >
        ...
        <Typography variant="subtitle1">{!isDemo?(product?.traits?.[3]?.value): (product?.price)}$</Typography>
        <Box display="flex" >
          <Box
            display="flex"
